...
class authValidation {
    ...
    static register = async (req,res,next)=>{
        try{
            ...
            }).validateAsync(req.body) //TODO: boddyden gelen kısımlar burada var mı dıye kontrol edip istenilemn tipte olup olmadığının kontrolününü gerçekleştirir
        }
        catch(error){
            if(error.detail[0] && error?.detail[0].message)
                throw new APIError(error.detail[0].message,400)
            else throw new APIError("LÜTFEN VALİDASYON KURALLARINA UYUN",400)
        }
        next()//TODO: ara yazılım olduğu için devam etmesi için yaptık
    }
}
